<template>
    <div ref="select" :class="classes">
        ...
        <select ref="sel" v-model="value" v-show="show" :name="name" class="secret" :multiple="multiple"
                :required="required" :readonly="readonly" :disabled="disabled">
            <option v-if="required" value=""></option>
            <option v-for="option in options" :value="option[optionsValue]||option">{{ option[optionsLabel]||option }}
            </option>
        </select>
        ...
        <div v-if="showNotify && closeOnSelect" class="notify out" transition="fadein">
            <div>{{limitText}}</div>
        </div>
    </div>
</template>
        ...
        computed: {
            ...
            filteredOptions () {
                ...
                return self.options.filter(function (option) {
                    return option.indexOf(self.searchValue) !== -1
                })
            }
        },
        watch: {
